...
{
    public class SemtControl : SearchEmlakBase
    {
        public override void ExecuteProcess(SerchChanRequest _search)
        {
            if (base.sonuc)//bir önceki adımdan geçiyorsa
            {
                ...
                if (_search.request.SemtId != 0)
                    base.sonuc = true;
                else
                {
                    EmlakKategoriler kat = _search._emlakKategoriler.GetParametreIlan(x => x.Id == _search.request.SatilikId).First();
