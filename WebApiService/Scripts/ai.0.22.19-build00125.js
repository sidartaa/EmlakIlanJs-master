...
(function (Microsoft) {
    ...
    (function (ApplicationInsights) {
        ...
        (function (_InternalMessageId) {
            ...
            _InternalMessageId[_InternalMessageId["NONUSRACT_BrowserCannotReadlocalStorage"] = 2] = "NONUSRACT_BrowserCannotReadlocalStorage";
            ...
            _InternalMessageId[_InternalMessageId["NONUSRACT_BrowserCannotWritelocalStorage"] = 4] = "NONUSRACT_BrowserCannotWritelocalStorage";
            ...
            _InternalMessageId[_InternalMessageId["NONUSRACT_BrowserFailedRemovalFromlocalStorage"] = 6] = "NONUSRACT_BrowserFailedRemovalFromlocalStorage";
        ...
        var _InternalLogging = (function () {
            ...
            _InternalLogging.clearInternalMessageLoggedTypes = function () {
                if (ApplicationInsights.Util.canUselocalStorage()) {
                    var localStorageKeys = ApplicationInsights.Util.getlocalStorageKeys();
                    for (var i = 0; i < localStorageKeys.length; i++) {
                        if (localStorageKeys[i].indexOf(_InternalLogging.AIInternalMessagePrefix) === 0) {
                            ApplicationInsights.Util.removelocalStorage(localStorageKeys[i]);
                        }
                    }
                }
            };
            ...
            _InternalLogging.logInternalMessage = function (severity, message) {
                ...
                if (ApplicationInsights.Util.canUselocalStorage()) {
                    ...
                    var internalMessageTypeLogRecord = ApplicationInsights.Util.getlocalStorage(storageMessageKey);
                    if (internalMessageTypeLogRecord) {
                        logMessage = false;
                    }
                    else {
                        ApplicationInsights.Util.setlocalStorage(storageMessageKey, "1");
                    }
                }
                if (logMessage) {
    ...
    (function (ApplicationInsights) {
        ...
        var Util = (function () {
            ...
            Util._getVerifiedStorageObject = function (storageType) {
                ...
                try {
                    uid = new Date;
                    storage = storageType === StorageType.LocalStorage ? window.localStorage : window.localStorage;
            ...
            Util.getlocalStorageKeys = function () {
                var keys = [];
            ...
            Util.getlocalStorage = function (name) {
                ...
                if (storage !== null) {
                    try {
                        ...
                        var message = new ApplicationInsights._InternalLogMessage(ApplicationInsights._InternalMessageId.NONUSRACT_BrowserCannotReadlocalStorage, "Browser failed read of session storage. " + Util.getExceptionName(e), { exception: Util.dump(e) });
                        ApplicationInsights._InternalLogging.throwInternalNonUserActionable(ApplicationInsights.LoggingSeverity.CRITICAL, message);
                    }
                }
                return null;
            };
            Util.setlocalStorage = function (name, data) {
                var storage = Util._getlocalStorageObject();
                if (storage !== null) {
                    try {
                        storage.setItem(name, data);
                        ...
                        var message = new ApplicationInsights._InternalLogMessage(ApplicationInsights._InternalMessageId.NONUSRACT_BrowserCannotWritelocalStorage, "Browser failed write to session storage. " + Util.getExceptionName(e), { exception: Util.dump(e) });
                        ApplicationInsights._InternalLogging.throwInternalNonUserActionable(ApplicationInsights.LoggingSeverity.CRITICAL, message);
                    }
                }
                return false;
            };
            Util.removelocalStorage = function (name) {
                var storage = Util._getlocalStorageObject();
                if (storage !== null) {
                    try {
                        storage.removeItem(name);
                        ...
                        var message = new ApplicationInsights._InternalLogMessage(ApplicationInsights._InternalMessageId.NONUSRACT_BrowserFailedRemovalFromlocalStorage, "Browser failed removal of session storage item. " + Util.getExceptionName(e), { exception: Util.dump(e) });
                        ApplicationInsights._InternalLogging.throwInternalNonUserActionable(ApplicationInsights.LoggingSeverity.CRITICAL, message);
                    }
                }
                return false;
        ...
        var DataLossAnalyzer = (function () {
            ...
            DataLossAnalyzer.reset = function () {
                if (DataLossAnalyzer.isEnabled()) {
                    ApplicationInsights.Util.setlocalStorage(DataLossAnalyzer.ITEMS_QUEUED_KEY, "0");
            ...
            DataLossAnalyzer.isEnabled = function () {
                return DataLossAnalyzer.enabled &&
                    ...
                    ApplicationInsights.Util.canUselocalStorage();
            };
            DataLossAnalyzer.getIssuesReported = function () {
                var result = (!DataLossAnalyzer.isEnabled() || isNaN(+ApplicationInsights.Util.getlocalStorage(DataLossAnalyzer.ISSUES_REPORTED_KEY))) ?
                    0 :
                    +ApplicationInsights.Util.getlocalStorage(DataLossAnalyzer.ISSUES_REPORTED_KEY);
                return result;
            };
            DataLossAnalyzer.incrementItemsQueued = function () {
                try {
                    if (DataLossAnalyzer.isEnabled()) {
                        ...
                        ++itemsQueued;
                        ApplicationInsights.Util.setlocalStorage(DataLossAnalyzer.ITEMS_QUEUED_KEY, itemsQueued.toString());
                    }
                }
                catch (e) { }
            };
            ...
            DataLossAnalyzer.getNumberOfLostItems = function () {
                ...
                try {
                    if (DataLossAnalyzer.isEnabled()) {
                        result = isNaN(+ApplicationInsights.Util.getlocalStorage(DataLossAnalyzer.ITEMS_QUEUED_KEY)) ?
                            0 :
                            +ApplicationInsights.Util.getlocalStorage(DataLossAnalyzer.ITEMS_QUEUED_KEY);
                    }
                }
                catch (e) {
                    result = 0;
            ...
            DataLossAnalyzer.reportLostItems = function () {
                try {
                    if (DataLossAnalyzer.isEnabled() &&
                        ...
                        ++issuesReported;
                        ApplicationInsights.Util.setlocalStorage(DataLossAnalyzer.ISSUES_REPORTED_KEY, issuesReported.toString());
                    }
                }
                catch (e) {
...
(function (Microsoft) {
    ...
    (function (ApplicationInsights) {
        ...
        var localStorageSendBuffer = (function () {
            function localStorageSendBuffer(config) {
                this._config = config;
                var bufferItems = this.getBuffer(localStorageSendBuffer.BUFFER_KEY);
                var notDeliveredItems = this.getBuffer(localStorageSendBuffer.SENT_BUFFER_KEY);
                this._buffer = bufferItems.concat(notDeliveredItems);
                ApplicationInsights.DataLossAnalyzer.itemsRestoredFromSessionBuffer = this._buffer.length;
                this.setBuffer(localStorageSendBuffer.BUFFER_KEY, this._buffer);
                this.setBuffer(localStorageSendBuffer.SENT_BUFFER_KEY, []);
            }
            localStorageSendBuffer.prototype.enqueue = function (payload) {
                this._buffer.push(payload);
                this.setBuffer(localStorageSendBuffer.BUFFER_KEY, this._buffer);
            };
            localStorageSendBuffer.prototype.count = function () {
                return this._buffer.length;
            };
            localStorageSendBuffer.prototype.clear = function () {
                this._buffer.length = 0;
                this.setBuffer(localStorageSendBuffer.BUFFER_KEY, []);
                this.setBuffer(localStorageSendBuffer.SENT_BUFFER_KEY, []);
            };
            localStorageSendBuffer.prototype.getItems = function () {
                return this._buffer.slice(0);
            };
            localStorageSendBuffer.prototype.batchPayloads = function (payload) {
                if (payload && payload.length > 0) {
                    var batch = this._config.emitLineDelimitedJson() ?
                        payload.join("\n") :
                        "[" + payload.join(",") + "]";
            ...
            localStorageSendBuffer.prototype.markAsSent = function (payload) {
                var sentElements = this.getBuffer(localStorageSendBuffer.SENT_BUFFER_KEY);
                sentElements = sentElements.concat(payload);
                this._buffer = this.removePayloadsFromBuffer(payload, this._buffer);
                this.setBuffer(localStorageSendBuffer.BUFFER_KEY, this._buffer);
                this.setBuffer(localStorageSendBuffer.SENT_BUFFER_KEY, sentElements);
            };
            localStorageSendBuffer.prototype.clearSent = function (payload) {
                var sentElements = this.getBuffer(localStorageSendBuffer.SENT_BUFFER_KEY);
                sentElements = this.removePayloadsFromBuffer(payload, sentElements);
                this.setBuffer(localStorageSendBuffer.SENT_BUFFER_KEY, sentElements);
            };
            localStorageSendBuffer.prototype.removePayloadsFromBuffer = function (payloads, buffer) {
                var remaining = [];
                for (var i in buffer) {
                    var contains = false;
            ...
            localStorageSendBuffer.prototype.getBuffer = function (key) {
                try {
                    var bufferJson = ApplicationInsights.Util.getlocalStorage(key);
                    if (bufferJson) {
            ...
            localStorageSendBuffer.prototype.setBuffer = function (key, buffer) {
                try {
                    var bufferJson = JSON.stringify(buffer);
                    ApplicationInsights.Util.setlocalStorage(key, bufferJson);
                }
                catch (e) {
                    ApplicationInsights._InternalLogging.throwInternalNonUserActionable(ApplicationInsights.LoggingSeverity.CRITICAL, new ApplicationInsights._InternalLogMessage(ApplicationInsights._InternalMessageId.NONUSRACT_FailToSetStorageBuffer, " storage key: " + key + ", " + ApplicationInsights.Util.getExceptionName(e), { exception: ApplicationInsights.Util.dump(e) }));
                }
            };
            localStorageSendBuffer.BUFFER_KEY = "AI_buffer";
            localStorageSendBuffer.SENT_BUFFER_KEY = "AI_sentBuffer";
            return localStorageSendBuffer;
        })();
        ApplicationInsights.localStorageSendBuffer = localStorageSendBuffer;
    })(ApplicationInsights = Microsoft.ApplicationInsights || (Microsoft.ApplicationInsights = {}));
})(Microsoft || (Microsoft = {}));
/// <reference path="serializer.ts" />
/// <reference path="Telemetry/Common/Envelope.ts"/>
    ...
    (function (ApplicationInsights) {
        ...
        var Sender = (function () {
            function Sender(config) {
                ...
                this._config = config;
                this._sender = null;
                this._buffer = (ApplicationInsights.Util.canUselocalStorage() && this._config.enablelocalStorageBuffer())
                    ? new ApplicationInsights.localStorageSendBuffer(config) : new ApplicationInsights.ArraySendBuffer(config);
        ...
        (function (Telemetry) {
            ...
            var PageVisitTimeManager = (function () {
                ...
                PageVisitTimeManager.prototype.startPageVisitTimer = function (pageName, pageUrl) {
                    try {
                        if (ApplicationInsights.Util.canUselocalStorage()) {
                            if (ApplicationInsights.Util.getlocalStorage(this.prevPageVisitDataKeyName) != null) {
                            ...
                            ApplicationInsights.Util.setlocalStorage(this.prevPageVisitDataKeyName, currPageVisitDataStr);
                        }
                    }
                    catch (e) {
                ...
                PageVisitTimeManager.prototype.stopPageVisitTimer = function () {
                    try {
                        if (ApplicationInsights.Util.canUselocalStorage()) {
                            ...
                            var pageVisitDataJsonStr = ApplicationInsights.Util.getlocalStorage(this.prevPageVisitDataKeyName);
                            if (pageVisitDataJsonStr) {
                                var prevPageVisitData = JSON.parse(pageVisitDataJsonStr);
                                prevPageVisitData.pageVisitTime = pageVisitEndTime - prevPageVisitData.pageVisitStartTime;
                                ApplicationInsights.Util.removelocalStorage(this.prevPageVisitDataKeyName);
                                return prevPageVisitData;
                            }
                            else {
                                return null;
        ...
        var AppInsights = (function () {
            function AppInsights(config) {
                ...
                var configGetters = {
                    ...
                    enablelocalStorageBuffer: function () { return _this.config.enablelocalStorageBuffer; }
                };
                var enableExperiment = new ApplicationInsights.SplitTest().isEnabled(this.config.instrumentationKey, 5);
                this.config.enablelocalStorageBuffer = enableExperiment;
        ...
        var Initialization = (function () {
            ...
            Initialization.getDefaultConfig = function (config) {
                ...
                config.disableFlushOnBeforeUnload = (config.disableFlushOnBeforeUnload !== undefined && config.disableFlushOnBeforeUnload !== null) ?
                    ...
                    false;
                config.enablelocalStorageBuffer = (config.enablelocalStorageBuffer !== undefined && config.enablelocalStorageBuffer !== null) ?
                    ApplicationInsights.Util.stringToBoolOrDefault(config.enablelocalStorageBuffer) :
                    false;
                return config;
            };
